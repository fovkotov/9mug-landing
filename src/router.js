import { beginPageType, cancelPageTypewriter, flushPageType, typeShellOnce } from "./page-typewriter.js";
...
async function mount(file, page, { focus }) {
  ...
  try {
    ...
    if (generation !== navGeneration || !page.isConnected) return;
    ...
    resizeScroll();
    // Capture post-init copy (cart label, checkout lines), then type.
    beginPageType(page);
    flushPageType();
    window.dispatchEvent(new CustomEvent("spa:settled"));
  } catch (error) {
    if (generation === navGeneration) cancelPageTypewriter();
    throw error;
  }
}
